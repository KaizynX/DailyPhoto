# DailyPhoto 方案与调研记录

更新日期：2026-09-11

## 目标

在固定工位的 Windows 电脑上，以尽可能低的日常操作成本，记录实际来到工位的每一天。

确定的产品行为：

- 程序启动后常驻 Windows 通知区域；启动和每次屏幕解锁时检查当天是否已经拍摄。
- 当天照片成功保存后不再自动弹出拍摄窗口；用户仍可从托盘手动加拍。
- 拍摄前显示摄像头画面并倒计时，默认 10 秒。
- 拍摄后显示静态预览，允许保存或重拍。
- 预览确认界面默认 15 秒没有操作时自动保存，避免无人确认导致当天遗漏。
- 支持从托盘菜单随时拍照、修改参数、打开照片目录和退出。
- 通过当前用户的 Windows 启动项实现可开关的登录自启动。
- 通过无运行时限的会话解锁任务恢复意外退出的托盘进程。
- 当前阶段只保存本地；不会自动上传。

## 为什么选择自制轻量工具

调研过的现成方案各自只覆盖一部分需求：

- Windows 相机具有拍照、倒计时和查看最近照片等能力，但没有“每日首次解锁、当天去重、确认超时自动保存”这一完整工作流。
  - <https://support.microsoft.com/en-us/windows/how-to-use-the-windows-camera-app-ea40b69f-be6a-840e-9c8c-1fd6eea97c22>
- Yawcam 是 Windows 摄像头软件，主要面向定时拍摄、监控和流媒体，并不针对每日自拍确认流程。
  - <https://www.yawcam.com/>
- Journal2Day 能从电脑摄像头记录每日自拍，但它本质上是需要进入的日记软件，不能替代解锁触发。
  - <https://www.kabsoftware.com/journal2day/>
- AgeLapse 可以对长期积累的人脸照片进行特征点检测、对齐并生成稳定的成长延时视频，但它负责后期处理，不负责本方案的采集触发。
  - <https://github.com/hugocornellier/agelapse>

因此，本项目只实现缺失的“可靠触发 + 当日去重 + 轻量确认”部分，并使用普通 JPEG 作为开放的数据格式，避免将照片锁在某个应用数据库里。

## 运行设计

### 常驻与触发

程序使用系统托盘作为常驻入口。启动后的行为如下：

- 当天没有照片时，自动打开拍摄窗口；
- 当天已经有照片时，只显示托盘图标；
- 每次收到当前 Windows 会话的解锁通知时重新检查，当天尚未拍摄才打开窗口；
- 用户可以从托盘菜单选择“立即拍照”，不受当天完成状态限制；
- 拍照窗口关闭或照片保存后，程序继续在托盘运行。

启用“开机自启动”后，程序写入当前用户的 Windows `Run` 启动项，并创建无运行时限的登录/会话解锁恢复任务。两个入口均可在登录时启动程序；任一入口成功启动后会检查并补回缺失的另一入口。常驻进程正常时由内部监听器处理解锁，恢复任务启动的重复实例会因单实例互斥立即退出；常驻进程意外结束时，恢复任务会在下次解锁重新启动程序。关闭自启动时会同时移除启动项和恢复任务。

### 当天完成判定

照片默认保存在（根目录可以在设置中修改）：

```text
photos/YYYY/MM/YYYY-MM-DD_HH-mm-ss.jpg
```

当日目录中存在符合日期前缀的 JPEG，即认为当天已经完成。只有文件成功写入后才成立。拍摄和确认过程中不会提前写入“完成”标志。

通过 `--force` 启动时忽略当天完成检查，用于测试；测试保存的照片仍然是正常照片。

### 拍摄交互

1. 打开摄像头并显示镜像实时预览。
2. 画面稳定期间进行 10 秒倒计时。
3. 从最新画面拍照，显示静态预览。
4. 用户可保存或重拍。
5. 15 秒无选择则自动保存。

如果摄像头正被会议软件独占，程序会显示错误而不会把当天标记为完成；可以关闭占用摄像头的软件后，从托盘菜单再次拍照。

### 托盘菜单

- `立即拍照`：打开拍摄窗口；双击托盘图标也会执行此操作。
- `参数设置…`：修改并保存运行参数。
- `打开照片目录`：在资源管理器中打开当前保存根目录。
- `开机自启动`：切换当前用户的登录启动项。
- `退出`：释放摄像头、移除托盘图标并结束程序。

### 本地性和隐私

- 摄像头帧只在本机内存中处理。
- 程序没有网络上传代码。
- 摄像头指示灯会在预览和拍摄时亮起。
- 文件夹中的照片是普通 JPEG，可随时复制、查看或迁移。

## 配置

`config.json` 中的主要选项：

- `photos_directory`：照片根目录；相对路径以项目根目录为基准，也支持绝对路径。
- `capture_delay_seconds`：拍摄前倒计时，默认 10。
- `confirmation_timeout_seconds`：拍摄后默认保存超时，默认 15。
- `camera_index`：摄像头编号，默认 0。
- `mirror_image`：预览和保存是否镜像，默认 true。
- `jpeg_quality`：JPEG 质量，默认 95。

这些选项通常无需手工编辑，可通过托盘菜单的“参数设置…”修改。写入配置时使用临时文件替换，避免保存中断留下不完整 JSON。

## 诊断

程序将启动、摄像头打开、取帧、保存和未处理异常写入项目根目录的 `daily_photo.log`。日志最大为 512 KB，并保留一个历史文件；日志不包含照片内容。

## 云备份：未来扩展

当前版本只存本地。未来建议按以下优先级扩展：

1. 将 `photos` 文件夹同步到 OneDrive 等常规云盘。
2. 定期复制到移动硬盘，形成与同步服务独立的恢复副本。
3. GitHub 私有仓库只作为可选的额外副本，不作为主要照片云。

GitHub 对普通 Git 对象强制限制为 100 MB，并建议控制仓库总大小；大量二进制照片会让完整克隆逐年变慢。Git LFS 更适合二进制，但个人免费计划的存储和下载配额有限，恢复体验也不如网盘。

- GitHub 仓库限制：<https://docs.github.com/en/repositories/creating-and-managing-repositories/repository-limits>
- Git LFS 说明：<https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-git-large-file-storage>
- Git LFS 计费和免费配额：<https://docs.github.com/en/billing/managing-billing-for-your-products/managing-billing-for-git-large-file-storage/upgrading-git-large-file-storage>

如果以后加入云备份，应默认关闭，并明确显示目标服务、最近成功时间及失败状态。

## 人脸对齐延时影像

此前调研的 AgeLapse 仍适合需要人工调整和相册管理的场景：

- 检测眼睛等人脸特征点；
- 对照片做缩放、旋转和平移对齐；
- 生成稳定的 photo-a-day 成长延时视频；
- 支持 Windows 桌面。

项目现已提供内置批处理工作流：使用 OpenCV YuNet 检测眼睛和嘴角，以相似变换统一人脸位置、缩放与旋转，再从同一批对齐帧生成 MP4、GIF 或两者。托盘菜单中的生成窗口可选择正方形人脸特写或保留更多环境的 16:9 宽景，并可在每帧左下角叠加拍摄日期。所有派生文件写入 `generated`，不替换原始 JPEG；无法检测人脸的照片会记录在 `manifest.json` 中。AgeLapse 仍可作为需要手动微调时的替代工具。

## 后续候选功能

- 云盘备份及失败提醒；
- 月度日历视图和照片浏览；
- 每月或每年自动生成延时影像；
- 摄像头名称选择界面（当前版本使用摄像头编号）；
- 拍摄成功后的 Windows 通知；
- 照片完整性校验和独立离线备份提醒。
